describe("your regexes", function(){
  ...
  it("should match ip addresses", function(){
    ...
    var regex = /\d{0,3}[.]\d{0,3}[.]\d{0,1}[.]\d{0,3}/;
  ...
  it("should detect nearby words", function(){
    ...
    var containsNearbyWords = function(haystack, needle){
      ...
      while (haystackWords = regex.exec(haystack)) {
        results.push(haystackWords[0]);
        results.shift();
        if (_.filter(results, function(value) { return value === needle }).length > 1) {
          return true;
        }
      }
      return false;
